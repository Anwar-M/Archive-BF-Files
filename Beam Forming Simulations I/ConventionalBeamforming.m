function [A,a] = ConventionalBeamforming(recorded_signal, sample_frequency, ...
...
%         g = (-exp(-2.*pi.*1i.*imaged_frequencies(K).*xi_timedelay)) ./ (4*pi*r);
        ...
        R_adapted = sqrt( dot(M_array,r2,3).^2 + beta_sq.*dot(r2,r2,3) );
    ...
    a = Numerator./Denominator; % complex amplitude of monopole for certain frequency
    A(:,:,K) = dot(Numerator./Denominator, Numerator./Denominator, 3)/2; % source autopower
    Numerator(:,:) = 0;
    Denominator(:,:) = 0;
end

% calculate p_rms by sqrt, p_rms = amplitude/sqrt(2) if multiple
% frequencies
% A = (sum(sqrt(1.*A),3)/n_freqs).^2;
% A = sqrt(sum(A.^2, 3)/2);
% A = sum((sqrt(2.*A)/n_freqs).^2,3)/2;
